...
race_results <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/results2021.csv')
starting_grid <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/starts2021.csv')
fastest_laps <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/flaps2021.csv')
sprint_results <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/sprints2021.csv')
qualis <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/qualis2021.csv')
gp_names <- read_csv('/Users/marcohui/Desktop/F1 Data Project/f1-2021-titlefight/data/names2021.csv')
...
race_results %>%
  # ignore non-finishing positions
...
*Note: calculations exclude sprint race results*
...
Although Verstappen and Hamilton almost match each other in topping the timing sheets during qualifying sessions, Verstappen ends up starting the race in pole position significantly more often. This can be explained by Hamilton losing positions at sprint races and taking an engine penalty while ahead, both of which Verstappen never did.
...
race_results_s %>% 
  ...
  filter(endPos=='1') %>%
  mutate(endPos=as.numeric(endPos)) %>%
...
In fact, when Hamilton is on pole position because of a good qualifying performance, he is just as likely to achieve a win compared to Verstappen. The difference between two in pole conversion rate is much lower than that in percentage of wins from poles.
...
# Change in Position [WORK IN PROGRESS]

This describes how the races played out.

## Average Position Gained or Lost
We can look at average position gained or lost throughout all 22 races. However, this is not the best metric of performance because it does not consider starting positions. Verstappen and Hamilton, who often start in the front of the grid, have much fewer opportunities to gain positions; whereas hypothetically, a driver starting from last has 19 cars to overtake.

The only 'real' insight we can draw from this statistic is that Hamilton loses fewer positions during a race than Verstappen does on average.

```{r Average Positions Gained or Lost}
race_results_s <- race_results %>% 
  select(Driver,gp_name,Pos) %>%
  rename(endPos=Pos)
...
startend <- race_results_s %>% 
  ...
  mutate(endPos=replace(endPos,endPos=='NC','21'), # DNF/DNS
         endPos=replace(endPos,endPos=='DQ','100'), # disqualification
...
startend %>%
  ...
  group_by(Driver) %>%
  summarise(avgDifPos=round(mean(difPos),3)) %>%
  arrange(-avgDifPos)
```

Let's see how the positions gained and lost played out race by race for Verstappen and Hamilton.

```{r Positions Gained or Lost}
race_results_s <- race_results %>% 
  select(Driver,gp_name,Pos) %>%
  rename(endPos=Pos)
...
startend %>%
  ...
  ggplot() + geom_bar(aes(x=gp_name,y=difPos),stat='identity') + coord_flip() + facet_wrap(~Driver)
```
...
haha <- startend %>% 
  ...
  gather(key='var',value='value',3:4) %>%
  filter(Driver=='Lewis Hamilton HAM') %>%
  arrange(value)

ggplot(data=haha) + 
  geom_point(aes(x=gp_name,y=value,fill=var),size=3,pch=21,colour=alpha("white",0)) +
  geom_linerange(data=haha,aes(x=gp_name,ymin=startPos,ymax=endPos)) +
...
# add point finishes in sprint results to race_results
...
A breakdown of points scored per Grand Prix allows for a more detailed view of how the drivers performed throughout the season. For instance, the troughs on the dark blue line see Verstappen scoring minimal points in Azerbeijan, Great Britain, Hungary, and Italy mostly due to retirements or technical issues. The extended peak from France to Autria represents a particular run of good form. 

On the light blue line, the highest point at the middle shows Lewis Hamilton achieving more points than any other weekend due to a second place finish at the sprint and a victory at the race. The three race peak covering Sao Paolo, Qatar, and Saudi Arabia portrays a slew of good results that helped him catch up to Verstappen in the championship. The datapoints in the middle section represents several mediocre results.
...
# Some Key Takeaways [WORK IN PROGRESS]
hello
